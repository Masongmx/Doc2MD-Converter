    ...
    private static FormatCheckReport CheckDocxFormat(string docxPath, string templateId)
    {
        ...
        try
        {
            ...
            if (pageSize == null || pageSize.Width?.Value != 11906 || pageSize.Height?.Value != 16838)
                issues.Add(new FormatCheckIssue { Code = "F_PAGE_SIZE", Severity = "medium", Message = "页面尺寸非 A4" });

            var bodyRuns = body.Descendants<Paragraph>()
                .Where(p => p.ParagraphProperties?.ParagraphStyleId?.Val?.Value == "Normal"
                         || p.ParagraphProperties?.ParagraphStyleId == null)
                .SelectMany(p => p.Elements<Run>())
                .ToList();

            var nonFangSongRuns = bodyRuns
                .Where(r => r.RunProperties?.RunFonts?.EastAsia?.Value != null
                         && r.RunProperties.RunFonts.EastAsia.Value != "仿宋"
                         && !string.IsNullOrEmpty(r.InnerText))
                .Take(5)
                .ToList();

            if (nonFangSongRuns.Count > 0)
                issues.Add(new FormatCheckIssue { Code = "F_BODY_FONT", Severity = "low", Message = $"正文存在非仿宋字体段落（共 {nonFangSongRuns.Count} 处）" });
